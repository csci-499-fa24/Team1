...
async function upsertRestaurantData(data) {
    for (const item of data) {
        try {
            ...
            const inspectionExists = await Inspections.findOne({ where: { camis: item.camis, inspection_date: item.inspection_date } });
            ...
            if (!inspectionExists) {
                ...
                await Inspections.update({
                    ...
                    grade_date: item.grade_date,
                    inspection_type: item.inspection_type,
                }, { where: { camis: item.camis, inspection_date: item.inspection_date } });
